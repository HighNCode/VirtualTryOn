...
export function Step2FrontPose({ onNext }: Props) {
  const uploadRef = useRef<HTMLInputElement>(null);

  return (
    <div className="flex flex-col items-center px-6 py-8 gap-5">
      <h2 className="text-xl font-bold text-[#1a1a1a] self-start">
        Position: <span className="text-[#E40206]">Front Pose</span>
      </h2>

      <div className="grid grid-cols-2 gap-4 w-full">
        {[
          { label: "Good Example", bg: "#f0f0f0", good: true },
          { label: "Bad Example", bg: "#f0f0f0", good: false }
        ].map((example) => (
          <div key={example.label} className="flex flex-col gap-2">
            <p className="text-xs text-[#6b7280] text-center">{example.label}</p>
            <div
              className="w-full aspect-[3/4] rounded-[14px] border border-[#e5e5e5] flex items-center justify-center overflow-hidden"
              style={{ background: example.bg }}
            >
              <img
                src={example.good ? "/widget/front-good.jpg" : "/widget/front-bad.jpg"}
                alt={example.label}
      ...
      <div className="flex flex-col gap-3 w-full">
        ...
        >
          <Upload size={17} />
          Upload <span className="text-[#E40206]">Front Pose</span>
        </button>
      </div>
    </div>
  );
