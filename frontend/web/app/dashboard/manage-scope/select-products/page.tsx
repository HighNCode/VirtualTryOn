...
export default function DashboardSelectProductsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [initialProducts, config] = await Promise.all([
        ...
        setSelectedIds(config.enabled_product_ids);
        setEnabledCollectionIds(config.enabled_collection_ids);
    ...
    try {
      await updateWidgetConfig({
        ...
        payload: {
          ...
          enabled_collection_ids: enabledCollectionIds,
          enabled_product_ids: selectedIds
        }
      });
      router.push("/dashboard/manage-scope");
    } catch (error: unknown) {
